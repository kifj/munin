 ...
 */
public abstract class AbstractMultiGraphsProvider extends AbstractGraphsProvider {
  ...
  public void printConfig(PrintWriter out) {
    ...
    for (Map.Entry<String, AbstractGraphsProvider> entry : providers.entrySet()) {
   ...
   * <code>Map</code> multiple times: Once as a non-graphing data graph and any
   * number of times as alias graphs.
   * 
   * @param providers
   ...
   */
  protected void addWithAlias(Map<String, AbstractGraphsProvider> providers, AbstractGraphsProvider provider,
      String dataName, String... aliases) {
  ...
  /**
   * Create a provider that works exactly like the specified provider, but adds
   * a <code>multigraph</code> attribute to the configuration, effectively
   * turning it into a multigraph provider.
   */
  public AbstractMultiGraphsProvider multigraph(final String name, final AbstractGraphsProvider provider) {
    return multigraph(name, provider, config);
  }

  public static AbstractMultiGraphsProvider multigraph(final String name, final AbstractGraphsProvider provider,
      Config config) {
  ...
  /**
   * Create a provider that works exactly like the specified provider, but adds
   * <code>graph no</code> to the configuration.
   */
  public AbstractGraphsProvider noGraph(final AbstractGraphsProvider provider) {
    return noGraph(provider, config);
  }

  public static AbstractGraphsProvider noGraph(final AbstractGraphsProvider provider, Config config) {
   ...
   */
  public AbstractGraphsProvider aliasFor(final AbstractGraphsProvider provider, final String origName) {
    return aliasFor(provider, getPrefix() + origName, config);
  }

  private static final Pattern FIELD_CONFIG_PATTERN = Pattern.compile("^([a-zA-Z0-9_]+)\\.", Pattern.MULTILINE);
  private static final Pattern GRAPH_ORDER_PATTERN = Pattern.compile("^graph_order .*$", Pattern.MULTILINE
   ...
   *         name (i.e. borrows all data from that provider)
   */
  public static AbstractGraphsProvider aliasFor(final AbstractGraphsProvider provider, final String origName,
      final Config config) {
    return new AbstractGraphsProvider(config) {
      ...
      public void printConfig(PrintWriter out) {
        ...
        while (matcher.find()) {
          if (graphConfig == null) {
            graphConfig = config.substring(0, matcher.start()).replaceFirst("\n+$", "");
