...
public abstract class GarbageCollectionInfo extends AbstractAnnotationGraphsProvider {
  ...
  private Collection<GarbageCollectorMXBean> getCollectors() {
    if (collectors == null) {
      try {
        ...
        collectors = new ArrayList<GarbageCollectorMXBean>(mbeans.size());
        for (ObjectName objectName : mbeans) {
          GarbageCollectorMXBean collector = ManagementFactory.newPlatformMXBeanProxy(getConnection(),
  ...
  protected void printFieldsConfig(PrintWriter out) {
    for (GarbageCollectorMXBean collector : getCollectors()) {
      ...
      printFieldConfig(out, name, label, info, "DERIVE", null, null, 0, Double.NaN);
    }
  }
  ...
  public static class Count extends GarbageCollectionInfo {
    ...
    protected String getFieldInfo(GarbageCollectorMXBean collector) {
      return "Number of collections that have occurred of the " + getCollectorInfo(collector);
    }
  ...
  public static class Time extends GarbageCollectionInfo {
    ...
    @Override
    protected String getFieldInfo(GarbageCollectorMXBean collector) {
      return "Approximate collection elapsed time in milliseconds of the " + getCollectorInfo(collector);
    }
