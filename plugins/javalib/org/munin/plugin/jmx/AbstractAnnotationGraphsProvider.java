...
public abstract class AbstractAnnotationGraphsProvider extends AbstractGraphsProvider {
  ...
  protected void printGraphConfig(PrintWriter out) {
    ...
    if (graph == null) {
      throw new IllegalArgumentException(getClass() + " doesn't have @Graph annotation");
    }
    String graphTitle = "JVM (port " + config.getPort() + ") " + graph.title();
    printGraphConfig(out, graphTitle, graph.vlabel(), graph.info(), graph.args(), graph.update(), graph.graph());
  }
  ...
  protected void printFieldConfig(PrintWriter out, AccessibleObject accessible) {
    ...
    printFieldConfig(out, fieldName, fieldLabel, field.info(), field.type(), field.draw(), field.colour(), field.min(),
        field.max());
  }

  private Collection<AccessibleObject> getFieldObjects() {
    // ensure methods are sorted by position first, then by name
    TreeSet<AccessibleObject> fieldObjects = new TreeSet<AccessibleObject>(new Comparator<AccessibleObject>() {
      ...
      public int compare(AccessibleObject m1, AccessibleObject m2) {
        ...
        if (result == 0) {
          result = getFieldName(m1).compareTo(getFieldName(m2));
        }
        return result;
      }
    });
  ...
  protected String getFieldName(AccessibleObject accessible) {
    ...
    if (fieldName.length() == 0) {
      ...
      if (accessible instanceof Method) {
        ...
        throw new IllegalArgumentException("AccessibleObject must be a field or a method: " + accessible);
      }
  ...
  private Object getFieldValue(AccessibleObject accessible) throws IllegalAccessException, InvocationTargetException {
    Object value;
    if (accessible instanceof Method) {
