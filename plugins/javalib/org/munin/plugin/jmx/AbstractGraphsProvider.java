...
public abstract class AbstractGraphsProvider {
  ...
  protected MBeanServerConnection getConnection() {
    if (connection == null) {
      try {
        ...
        throw new IllegalStateException("Failed to get MBean Server Connection", e);
      } catch (IOException e) {
        throw new IllegalStateException("Failed to get MBean Server Connection", e);
      }
    }
  ...
  protected void printFieldConfig(PrintWriter out, String name, String label, String info, String type, String draw,
      String colour, double min, double max) {
  ...
  protected void printFieldAttribute(PrintWriter out, String fieldName, String attributeName, Object value) {
    if (value != null) {
  ...
  private static AbstractGraphsProvider getProvider(Class<? extends AbstractGraphsProvider> providerClass, Config config) {
    try {
      return providerClass.getConstructor(Config.class).newInstance(config);
    } catch (NoSuchMethodException e) {
      // just try default constructor
      try {
        return providerClass.newInstance();
      } catch (Exception e1) {
        throw new IllegalArgumentException("Can't instantiate provider with default constructor: " + providerClass, e);
      }
    } catch (Exception e) {
      throw new IllegalArgumentException("Can't instantiate provider with constructor accepting Config object: "
          + providerClass, e);
    }
  }
