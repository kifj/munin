...
public class MemoryPoolUsageProvider extends AbstractMemoryUsageProvider {
  ...
  public MemoryPoolUsageProvider(final Config config, final ObjectName poolName, final UsageType usageType) {
    super(config);
  ...
  /*
   * We need to write the graph config manually, since it's dynamic and can't be
  ...
  protected void prepareValues() throws Exception {
    MemoryPoolMXBean pool = ManagementFactory.newPlatformMXBeanProxy(getConnection(), poolName.getCanonicalName(),
